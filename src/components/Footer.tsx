
import { Mail, Phone, MapPin } from 'lucide-react';

const Footer = () => {
  return (
    <footer className="bg-olive text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <h3 className="text-xl font-semibold mb-4 text-gold">Callih</h3>
            <p className="text-secondary/80">
              Cuidados especializados em saúde, proporcionando atendimento de qualidade e personalizado.
            </p>
          </div>
          
          <div>
            <h3 className="text-xl font-semibold mb-4 text-gold">Contato</h3>
            <div className="space-y-3">
              <div className="flex items-center space-x-3 text-secondary/80">
                <Phone size={20} className="text-gold" />
                <span>(XX) XXXX-XXXX</span>
              </div>
              <div className="flex items-center space-x-3 text-secondary/80">
                <Mail size={20} className="text-gold" />
                <span>contato@callih.com</span>
              </div>
              <div className="flex items-center space-x-3 text-secondary/80">
                <MapPin size={20} className="text-gold" />
                <span>São Paulo, SP</span>
              </div>
            </div>
          </div>
          
          <div>
            <h3 className="text-xl font-semibold mb-4 text-gold">Links Rápidos</h3>
            <ul className="space-y-2 text-secondary/80">
              <li><a href="#" className="hover:text-gold transition-colors">Home</a></li>
              <li><a href="#" className="hover:text-gold transition-colors">Serviços</a></li>
              <li><a href="#" className="hover:text-gold transition-colors">Entrega</a></li>
              <li><a href="#" className="hover:text-gold transition-colors">Sobre Nós</a></li>
            </ul>
          </div>
        </div>
        
        <div className="border-t border-gold/20 mt-8 pt-8 text-center text-secondary/60">
          <p>&copy; {new Date().getFullYear()} Callih. Todos os direitos reservados.</p>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
